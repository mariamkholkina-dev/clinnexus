...
from sqlalchemy import ARRAY, Boolean, DateTime, Float, ForeignKey, Integer, String, Text, TypeDecorator
...
from app.db.models.studies import DocumentLanguageType, DocumentTypeType
...
class Topic(Base):
    ...
    applicable_to_json: Mapped[list[str]] = mapped_column(
        JSONB, nullable=False, server_default="[]"
    )
...
class TopicZonePrior(Base):
    """Приоритет зоны для топика по типу документа."""

    __tablename__ = "topic_zone_priors"
...
class HeadingBlockTopicAssignment(Base):
    """Привязка блока заголовка к топику для конкретной версии документа."""

    __tablename__ = "heading_block_topic_assignments"
    ...
    heading_block_id: Mapped[str] = mapped_column(Text, nullable=False)
    topic_key: Mapped[str] = mapped_column(Text, nullable=False)
    confidence: Mapped[float | None] = mapped_column(nullable=True)
    debug_json: Mapped[dict[str, Any] | None] = mapped_column(JSONB, nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
