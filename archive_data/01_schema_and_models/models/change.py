...
class ImpactItem(Base):
    ...
    affected_target_section: Mapped[str] = mapped_column(Text, nullable=False)
    ...
    @property
    def affected_section_key(self) -> str:
        """Обратная совместимость: affected_section_key → affected_target_section (deprecated)."""
        import warnings
        warnings.warn(
            "Использование affected_section_key устарело. Используйте affected_target_section.",
            DeprecationWarning,
            stacklevel=2
        )
        return self.affected_target_section
    
    @affected_section_key.setter
    def affected_section_key(self, value: str) -> None:
        """Обратная совместимость: affected_section_key → affected_target_section (deprecated)."""
        import warnings
        warnings.warn(
            "Использование affected_section_key устарело. Используйте affected_target_section.",
        ...
        self.affected_target_section = value
