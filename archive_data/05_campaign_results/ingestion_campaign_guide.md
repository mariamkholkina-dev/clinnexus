...
- **Passports seed** (контракты секций) — изменения в `section_contracts` через API или seed-скрипт
...
- `backend/app/data/section_taxonomy_protocol.json` — таксономия секций для протоколов

**Проверка наличия:**
```bash
ls -la backend/app/data/source_zone_rules.yaml
ls -la backend/app/data/section_taxonomy_protocol.json
```
...
**Вероятные причины:**
- В taxonomy нет алиасов для часто встречающихся заголовков
...
**Где исправить:**
- **Taxonomy**: `backend/app/data/section_taxonomy_protocol.json` (добавить алиасы)
...
3. Добавить алиасы в taxonomy или улучшить правила маппинга
