...
from app.db.enums import FactStatus, SectionMapStatus, SourceZone
...
class MetricsCollector:
    ...
    async def collect_section_maps_metrics(
        ...
        stmt = select(func.count(SectionMap.id)).where(
            SectionMap.doc_version_id == UUID(self.doc_version_id)
        )
        result = await self.db.execute(stmt)
        ...
        if core_sections:
            ...
            stmt = select(func.count(SectionMap.id)).where(
                SectionMap.doc_version_id == UUID(self.doc_version_id),
                SectionMap.target_section.in_(core_section_keys),
                SectionMap.status.in_([SectionMapStatus.MAPPED, SectionMapStatus.NEEDS_REVIEW]),  # Учитываем только не-overridden
            )
            result = await self.db.execute(stmt)
        ...
        stmt = select(
            SectionMap.status,
            func.count(SectionMap.id).label("count"),
        ).where(SectionMap.doc_version_id == UUID(self.doc_version_id)).group_by(SectionMap.status)
        ...
        stmt = select(
            SectionMap.target_section,
            SectionMap.status,
            SectionMap.confidence,
        ).where(SectionMap.doc_version_id == UUID(self.doc_version_id))
