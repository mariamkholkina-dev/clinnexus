...
class SoAExtractionService:
    ...
    def _extract_soa_from_table(
        ...
        for idx, label in enumerate(visit_labels):
            ...
            # Формируем anchor_id без section_path для избежания превышения лимита в 512 символов
            # Используем table_index, row_idx, col_idx вместо section_path и ordinal
            anchor_id = f"{doc_version_id}:cell:{table_index}:{row_idx}:{col_idx}:{text_hash}"
