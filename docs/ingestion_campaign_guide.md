...
**Примечание**: В текущей реализации кампания может работать без embeddings (chunks создаются, но векторизация может быть отложена). Кампания ингестии **НЕ использует LLM** для основной обработки, но может использовать LLM-assist для проблемных секций при маппинге (если настроены API ключи и `SECURE_MODE=true`). Также LLM может использоваться в `ValueNormalizer` для GxP Double Check сложных значений фактов (если настроено).
...
- Через скрипт кампании: `--upload-file` или `--upload-dir` (см. раздел 4.1). Скрипт автоматически создает Study, Document и DocumentVersion для каждого файла
- Или через seed-скрипт (если есть тестовые данные)

**Примечание:** При использовании `--upload-file` или `--upload-dir` скрипт автоматически:
- Создает Study (если не указан `--study-code`, генерируется автоматически)
- Создает Document для каждого файла
- Создает DocumentVersion с версией "v1.0"
- Сохраняет файл в хранилище (`backend/.data/uploads/{version_id}/{filename}`)
...
**Загрузка и обработка директории с файлами (рекурсивный поиск):**
```bash
cd backend
python -m app.scripts.run_ingestion_campaign \
  --workspace-id <workspace_uuid> \
...
**Примечание:** Поддерживаемые форматы файлов: `.docx`, `.pdf`, `.xlsx`. При загрузке через `--upload-file` или `--upload-dir` скрипт автоматически создает Study, Document и DocumentVersion для каждого файла.
...
- `--dry-run` — режим проверки без реальной ингестии (показывает первые 10 документов, которые будут обработаны)
...
- `--upload-file <path>` — путь к файлу для загрузки (DOCX, PDF, XLSX). При указании этого параметра файл будет загружен в БД и затем обработан. Скрипт автоматически создает Study, Document и DocumentVersion
- `--upload-dir <path>` — путь к директории с файлами для загрузки (рекурсивный поиск). Поддерживаемые форматы: DOCX, PDF, XLSX. Для каждого найденного файла создается отдельный Document и DocumentVersion
- `--study-code <code>` — код исследования для создаваемых исследований (опционально, по умолчанию генерируется автоматически в формате `CAMPAIGN-YYYYMMDD-HHMMSS-<uuid>`)
...
- Использование LLM-assist для проблемных секций (если настроено и `SECURE_MODE=true`)
- Topic mapping (только для протоколов, добавляет время обработки)
- Выравнивание якорей с предыдущей версией документа (если есть)
- Проверка согласованности фактов (FactConsistencyService)
...
```json
{
  ...
  "avg_topics_rate": 0.78,
  "topics_rate_below_50pct": 15,
  "top_warnings": {...},
  "top_errors": {...},
  "section_failures": {...}
}
...
- `topics_rate` — доля блоков заголовков с назначенным топиком (только для протоколов)
...
#### Topics mapping rate (только для протоколов)

**Пороги:**
- **> 50%** — хорошо (больше половины блоков заголовков имеют назначенный топик)
- **30-50%** — приемлемо, но стоит улучшить правила маппинга топиков
- **< 30%** — требует внимания

**Где смотреть:**
- `summary_json.topics.mapped_rate` (в `ingestion_runs.summary_json`)
- `campaign_summary.avg_topics_rate`
- `campaign_details.csv.topics_rate`
...
### 10.8 Проблемы на Windows

**Особенности работы на Windows:**
- Скрипт автоматически использует `WindowsSelectorEventLoopPolicy` для совместимости с psycopg (async драйвер PostgreSQL)
- Пути к файлам обрабатываются корректно (поддержка `file:///` URI и относительных путей)
- При использовании `--upload-dir` поиск файлов выполняется рекурсивно во всех подкаталогах

**Если возникают проблемы с путями:**
- Используйте абсолютные пути или относительные пути от текущей рабочей директории
- Убедитесь, что пути не содержат недопустимых символов
...
**Примечания**: 
- Правила source_zone теперь хранятся в отдельных файлах для каждого типа документа: `backend/app/data/source_zones/rules_{doc_type}.yaml`
- Скрипт поддерживает загрузку файлов форматов DOCX, PDF, XLSX через параметры `--upload-file` и `--upload-dir`
- При загрузке файлов скрипт автоматически создает Study, Document и DocumentVersion
- Скрипт обрабатывает документы последовательно (concurrency=1) для безопасности и предотвращения блокировок БД
- Для протоколов выполняется topic mapping (маппинг блоков заголовков на топики), метрики доступны в `topics_rate`
