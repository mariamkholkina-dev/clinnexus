...
def main() -> None:
    ...
    ap.add_argument("--docx", default="", help="Path to protocol DOCX (needed if creating a new version)")
    ...
    if not version_id:
        ...
        if not docx_path:
            docx_path = input("Enter path to protocol DOCX (with headings / SoA if possible): ").strip().strip('"')
        assert_true(os.path.exists(docx_path), f"DOCX file not found: {docx_path}")
        assert_true(docx_path.lower().endswith(".docx"), f"Expected .docx file: {docx_path}")
        ...
        up = upload_file(api_base, version_id, docx_path)
        print(f"  upload sha256={up.get('sha256')} uri={up.get('uri')}")

        print("Starting ingestion...")
        try:
            # Ingestion может занимать много времени для больших документов
            # Увеличиваем таймаут до 120 секунд
            # Используем force=true для перезапуска, если ингестия уже была выполнена
        ...
        v = poll_ingest(api_base, version_id, timeout_sec=args.timeout)
        print(f"Final ingestion_status={v.get('ingestion_status')}")
    else:
        print(f"Using existing version_id={version_id}")
        # Для существующего version_id нужен workspace_id для создания контрактов
    ...
    if args.apply:
        ...
        for sk in section_keys:
            ...
            if not a:
                continue
            # overridden must not be touched, but we can't know here; we just detect changes.
            if not b:
                changed += 1
                continue
        ...
        # At least one section should be updated when QC passes. If none, still ok if QC rejected all.
